 ...
 * fa pai
 * */
gameLogicRemote.fapai = function(rid,channel,channelService,cb){
    ...
    if(users.length>=2){
            ...
            gameDao.getLocalPlayer(rid,i,function(err,res,location){
                if(res!='null'){
                    ...
                    gameDao.setIsGameNum(rid,location,1,function(err){
                        ...
                        //gameDao.getAllChip(rid,function(ex_all_chip){
                        //    gameDao.getCurrentChip(rid,function(cur_chip){
                        //        var new_chip = ex_all_chip+cur_chip;
                        //        gameDao.setAllChip(rid,new_chip,function(){
                        //            console.log("fapai:new_chip:"+new_chip);
                        //            console.log("fapai:setAllChip success");
                        //        });
                        //    });
        ...
        //    channelService.pushMessageByUids('onShoupai', param1, [{
        //        uid: users[i],
        //        sid: channel.getMember(users[i])['sid']
        //    }]);
        //}
 ...
 * 比牌请求处理路由
 * */
gameLogicRemote.bipai = function(rid,location1,location2,channel,cb){
    //比牌逻辑，返回结果
    var self = this;
 ...
 * */
gameLogicRemote.throw = function(rid,location,channel,username,channelService){
                        ...
                        delayDao.addDelay(rid,10,function(){
                            ...
                            gameDao.getIsGameNum(rid,function(err,isGameNumArr){
                                ...
                                if(sum<=1){
                                    ...
                                    gameLogicRemote.restartGame(rid,channel,channelService,game_winner);
 ...
 * @param channel
 */
gameLogicRemote.allin = function(rid,location,channel){
    gameDao.getIsGameNum(rid,function(err,isGameArr){
        var temp = 0;
        for(var i=0;i<5;i++){
 ...
 */
gameLogicRemote.restartGame = function(rid,channel,channelService,game_winner){
    //重新开始
    var param = {
        route:'onEnd',
        winner:game_winner
