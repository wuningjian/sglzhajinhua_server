 ...
 * 查询数据库，回调有空位房间序号
 * */
gameDao.returnRoom = function(cb){
    var sql = 'select * from game_room where player_num < ?';
    var args = [5];
    //var emptyRoom =[];
...
gameDao.resetData = function(rid,cb){
    ...
    var args =[null,null,null,null,null,100,0,0,0,0,0,0,0,0,0,null,0,0,rid];
    sqlTemp.update(sql,args,function(err,res){
        if(err!==null){
            console.error("db:resetData error");
            utils.invokeCallback(cb, err, null);
        }else{
